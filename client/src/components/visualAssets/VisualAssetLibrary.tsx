        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <label className="relative block min-w-0 flex-1">
            ...
            <Input
              ...
              placeholder="Search for material names, descriptions or prompt words"
          ...
          <div className="flex flex-wrap items-center gap-2">
            <FilterGroup label="Type" icon={<SlidersHorizontal className="h-3.5 w-3.5" aria-hidden="true" />}>
              <FilterButton active={!selectedKind} onClick={() => setSelectedKind(undefined)}>All</FilterButton>
            ...
            <FilterGroup label="Source">
              <FilterButton active={!selectedSource} onClick={() => setSelectedSource(undefined)}>All</FilterButton>
      ...
      <div className="flex min-h-0 flex-1">
        <div className="min-w-0 flex-1 overflow-y-auto">
          <div className="flex items-center justify-between gap-3 px-5 py-3 text-sm text-muted-foreground">
            <span>{typeof total === "number" ? `Found ${total} items` : "Preparing materials"}</span>
            {hasActiveFilters ? (
          ...
          <div className="px-5 pb-5">
            ...
            {catalogQuery.isError ? (
              <InlineState
                title="Unable to load visual material at the moment"
                description="Please check the connection and reload."
                actionLabel="reload"
            ...
            {!catalogQuery.isLoading && !catalogQuery.isError && items.length === 0 ? (
              <InlineState
                title="No matching visual material found"
                description={hasActiveFilters ? "Adjust your search or filters to continue viewing." : "After you finish creating the picture, the material will be displayed here."}
...
function FilterGroup({ label, icon, children }: { label: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex max-w-full items-center gap-1 overflow-x-auto rounded-md border bg-muted/[0.18] p-1" aria-label={`${label} filter`}>
...
function VisualAssetGridLoading() {
  return (
    <div className="columns-2 gap-3 sm:columns-3 xl:columns-4 2xl:columns-5" aria-label="Loading visual assets">
