...
export default function LiveExecutionDialog(props: LiveExecutionDialogProps) {
  ...
  const latestPreview = latestSession?.preview
    ...
    : "Waiting for the model to start returning content...";
  ...
  return (
    <>
      ...
      >
        ...
        {activeCount > 0 ? (
          <Badge className="ml-1.5 h-5 min-w-5 px-1.5 text-[10px]" aria-label={`${activeCount} Item AI generation in progress`}>
            {activeCount}
          </Badge>
        ) : null}
      </Button>
          ...
          >
            ...
            >
              ...
              <Badge variant="outline" className="shrink-0 border-emerald-400/50 bg-emerald-400/10 font-mono text-emerald-200">
                {activeCount > 0 ? `${activeCount} item in progress` : connected ? "Waiting for generation" : "Connecting"}
              </Badge>
              <Button
                type="button"
                variant="ghost"
                ...
                aria-label={briefMode ? "Switch to verbose mode" : "Switch to simple mode"}
                title={briefMode ? "View all calls" : "Only view the latest output"}
              >
                {briefMode ? <Maximize2 className="h-3.5 w-3.5" /> : <Minimize2 className="h-3.5 w-3.5" />}
                {briefMode ? "Detailed" : "simple"}
              </Button>
              {!briefMode ? (
              <Button
                type="button"
              ...
              <DialogPrimitive.Close asChild>
                <Button
                  ...
                  aria-label="Turn off AI creation live"
                  onPointerDown={(event) => event.stopPropagation()}
                  onPointerMove={(event) => event.stopPropagation()}
                  onPointerUp={(event) => event.stopPropagation()}
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="shrink-0 text-[11px] text-emerald-100/55">{session.totalChars.toLocaleString()} characters</span>
            ...
            <footer className={cn(
              ...
              <span>{followingLatest ? "Following the latest output" : "Already stuck at current reading position"}</span>
              {!briefMode ? (
