...
function artifactTypeLabel(type: string): string {
  const labels: Record<string, string> = {
    ...
    character_cast: "Character",
    volume_strategy: "Divide into volumes",
    chapter_task_sheet: "task order",
    chapter_draft: "Text",
    audit_report: "Reviewing",
    repair_ticket: "Repairing",
    ...
    <div className="space-y-4">
      <section className={cn("rounded-2xl p-5 shadow-sm", stateSoftSurfaceClassName(focusProjection.displayState))}>
        ...
        <div className="mt-5 grid gap-3 rounded-xl bg-background/60 p-3 sm:grid-cols-3">
          ...
          />
          <SummaryMetric label="Advancing Overview" value={focusProjection.progressSummary || "No progress summary yet"} />
          <SummaryMetric label="recent records" value={latestRecordText} />
        </div>

        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            <div className="text-[11px] text-muted-foreground">Next step</div>
      ...
      {circuitBreaker ? (
        <section className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm leading-6 text-destructive">
          <div className="font-medium">Auto-advance is paused</div>
          ...
          {circuitRecovery ? <div className="mt-1">Suggested next step: {circuitRecovery}.</div> : null}
        </section>
      ) : null}
      ...
      {workerHealth ? (
        <section className="rounded-2xl bg-muted/25 px-4 py-3">
          ...
          <div className="mt-3 grid grid-cols-4 gap-3">
            <SummaryMetric label="Queued" value={workerHealth.queuedCommandCount} />
            <SummaryMetric label="take over" value={workerHealth.leasedCommandCount} />
            <SummaryMetric label="execute" value={workerHealth.runningCommandCount} />
            <SummaryMetric label="restore" value={workerHealth.staleCommandCount} />
          </div>
      ...
      {usageSummary ? (
        <DetailPanel
          title="AI usage"
          summary={`${formatTokenCount(usageSummary.llmCallCount)} times · ${formatTokenCount(usageSummary.totalTokens)} Tokens`}
          ...
          <div className="space-y-3 text-xs leading-5 text-muted-foreground">
            ...
            {promptUsage.length > 0 ? (
              <div className="space-y-1">
                <div className="font-medium text-foreground">Stage dosage</div>
            ...
            {recentItems.map((item) => (
              <div key={item.id} className="py-2">
                ...
                {item.usage ? (
                  ...
                  <div className="mt-1 text-muted-foreground">Took {formatDuration(item.durationMs)}</div>
                ) : null}
