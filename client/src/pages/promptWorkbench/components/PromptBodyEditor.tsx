    ...
    >
      ...
      {remaining !== null ? (
        <div className="border-t border-[#dce8e4] bg-[#fbfdfb] px-3 py-1.5 text-right text-xs text-[#6f7f78]">
          {remaining < 0 ? <span className="text-destructive">{remaining}</span> : remaining} characters remaining
        </div>
      ) : null}
    </div>
  );
        ...
        <div className="min-w-0">
          ...
          {"anchor" in section.slot && section.slot.anchor ? (
            <p className="mt-1 text-xs text-muted-foreground">
              Anchor point:<code className="rounded bg-muted px-1">{section.slot.anchor}</code>
            </p>
          ) : null}
        </div>
        {canReset ? (
          <Button
            ...
            disabled={disabled}
            title={section.isOfficialDefaultOverride ? "Clear the official default mark of this book and re-inherit the global coverage" : "Clear current layer overlay"}
...
function reconcileStateLabel(item: PromptSlotReconcileItem): string {
  ...
  if (item.state === "new") return "Official new slots";
  return "Slot removed";
}

function displaySlotValue(value: string | boolean | undefined): string {
  if (value === undefined) return "None";
  if (typeof value === "boolean") return value ? "turn on" : "Close";
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-medium text-foreground">{checked ? "Enabled" : "Closed"}</span>
      </div>
      {checked ? (
  ...
  return (
    <div className="space-y-2">
      ...
      {"patternHint" in section.slot && section.slot.patternHint ? (
        <div className="text-xs text-muted-foreground">Expected format:{section.slot.patternHint}</div>
      ) : null}
    </div>
  );
}
      ...
      {!hasEditableSlots ? (
        ...
        <>
          {controlSections.length > 0 ? (
            <section className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-foreground">Operation control</h3>
              </div>
          ...
          {bodySections.length > 0 ? (
            <section className="space-y-3">
              <h3 className="text-sm font-semibold text-foreground">Prompt body slot</h3>
              <div className="space-y-3">
