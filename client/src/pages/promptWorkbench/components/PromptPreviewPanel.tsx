  ...
  if (isPending) {
    return (
      <div className="rounded-md border border-amber-200 bg-amber-50/75 p-4 text-sm text-amber-900">
        <div>{streamOutput ? "Continuously displaying model return content." : "Calling the model using the current draft, waiting for the first paragraph to return."}</div>
        {streamOutput ? (
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            {result.meta.provider ?? "prompt word routing"} / {result.meta.model ?? "Default model"} · {result.meta.latencyMs}ms
          </div>
        </div>
        <div className="flex flex-wrap gap-2 text-xs">
          <Badge variant="outline">{result.outputType === "structured" ? "structured" : "text"}</Badge>
          {usage ? (
          ...
          {result.meta.repairUsed ? (
            <Badge variant="outline">JSON fix {result.meta.repairAttempts ?? 0} times</Badge>
          ) : null}
        </div>
      </div>
