...
function severityLabel(severity?: QualityFlag["severity"]): string {
  ...
  return severity ? labels[severity] : "Tips";
}
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">First check the script quality, and then confirm the platform compliance risks.</div>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-6">
        ...
        <div className="rounded-md border p-3 text-sm">
          <div className="text-xs text-muted-foreground">Suggested fix</div>
              ...
              <CardHeader className="gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <CardTitle className="text-base">Episode {item.episode.order}: {item.episode.title}</CardTitle>
                    ...
                    {item.quality?.score?.overall != null ? (
                      <Badge variant="outline">Overall {item.quality.score.overall}</Badge>
                    ) : null}
                  </div>
          ...
          <CardContent className="grid gap-2 md:grid-cols-2">
            {uncheckedItems.map((item) => (
              <div key={item.episode.id} className="flex items-center justify-between gap-2 rounded-md border p-3 text-sm">
                <span>Episode {item.episode.order}: {item.episode.title}</span>
                <Button
                  type="button"
                  size="sm"
                  variant="outline"
