    ...
    <Card className="overflow-hidden rounded-2xl border-border/50 bg-card/80 shadow-[0_14px_40px_rgba(15,23,42,0.04)] backdrop-blur-sm">
      <CardHeader className="space-y-4 border-b border-border/40 px-4 pb-4 pt-5">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-lg tracking-tight">Analysis list</CardTitle>
      ...
      <CardContent className="space-y-3 px-3 pb-4 pt-4">
        <Input className="h-10 rounded-xl border-border/50 bg-muted/20 shadow-none" value={keyword} onChange={(event) => onKeywordChange(event.target.value)} placeholder="Search title or keyword" />
        <SelectControl
        ...
        <div className="space-y-1 pt-1">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    {item.publishedDocumentId && (
                      <Badge variant="secondary" className="border-0 bg-muted px-1.5 text-[10px] font-normal">Published</Badge>
                    )}
          ...
          {!loading && !errorMessage && analyses.length === 0 ? (
            <div className="rounded-md border border-dashed p-4 text-xs text-muted-foreground">
              {keyword.trim() || status
                ? "There is no split book analysis that matches the current filter. You can adjust the filter conditions."
                : "There is no open book analysis yet, click \"New Open Book\" above to start."}
            </div>
          ) : null}
        </div>
      </CardContent>
