...
function toStepTypeLabel(stepType: string): string {
  ...
  if (stepType === "completion") return "Finishing";
  if (stepType === "analysis") return "analysis";
  if (stepType === "review") return "Reviewing";
  if (stepType === "repair") return "Repairing";
...
function formatEvent(event: RuntimeEvent): string {
  if (event.type === "tool_call") {
    return `Call tool ${event.toolName}: ${event.inputSummary}`;
  }
  if (event.type === "tool_result") {
    return `${event.toolName} ${event.success ? "succeeded" : "failed"}: ${event.outputSummary}`;
  }
  if (event.type === "approval_required") {
    return `Awaiting approval: ${event.summary}`;
  }
  return `Approval results: ${toApprovalActionLabel(event.action)}${event.note ? ` (${event.note})` : ""}`;
}
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    void chatStore.createSession("New thread");
  ...
  const ensureSession = useCallback(async () => {
    ...
    return chatStore.createSession("New thread");
  }, [chatStore]);
    ...
    <div className="grid min-h-[70vh] gap-4 lg:grid-cols-[240px_minmax(0,1fr)_360px]">
      <Card>
        ...
        <CardContent className="space-y-2">
          <Button className="w-full" onClick={() => void chatStore.createSession("New thread")}>
            New conversation
          </Button>
              ...
              >
                ...
                {session.latestRunId ? (
                  <div className="text-[11px] text-muted-foreground">
                    Run: {session.latestRunId.slice(0, 8)} · {session.runIds?.length ?? 1} items
                  </div>
                ) : null}
              </button>
            ))}
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
          <div className="space-y-1">
            <CardTitle className="text-base">Conversation message</CardTitle>
            {headerRunMessage ? (
