    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm leading-6 text-muted-foreground">
              View the tasks, quality and repair basis of this chapter, which does not occupy the main text reading area.
            </div>
          </div>
          <Badge variant="outline" className="shrink-0">Chapter {selectedChapter.order}</Badge>
        </div>
      </div>
      ...
      <Tabs value={detailTab} onValueChange={(value) => onAssetTabChange(value as AssetTabKey)}>
        <TabsList className="grid h-auto w-full grid-cols-2 gap-1 rounded-2xl bg-muted/50 p-1.5">
          <TabsTrigger value="taskSheet" className="rounded-xl text-xs">task order</TabsTrigger>
          ...
          <TabsTrigger value="repair" className="rounded-xl text-xs">Repairing</TabsTrigger>
          <TabsTrigger value="content" className="col-span-2 rounded-xl text-xs">Contextual diagnostics</TabsTrigger>
        </TabsList>

        <TabsContent value="taskSheet" className="space-y-3">
          <div className="rounded-2xl border bg-muted/20 p-4">
            <div className="text-xs text-muted-foreground">Task sheet for this chapter</div>
            <div className="mt-3 whitespace-pre-wrap text-sm leading-7">
              {selectedChapter.taskSheet?.trim() || "There are no task orders yet. You can let the AI ​​generate the task list first and then come back and continue writing the chapter."}
            </div>
          </div>
          <PanelHintCard title="Chapter Objectives" content={chapterObjective} />
          <PanelHintCard title="latest status" content={latestStateSnapshot?.summary || "No status summary available yet."} />
          <ChapterRuntimeContextCard
        ...
        <TabsContent value="sceneCards" className="space-y-3">
          ...
          {scenePlan ? (
            <div className="space-y-3">
              <div className="rounded-2xl border bg-muted/20 p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <MetricBadge label="Chapter Objectives" value={`${scenePlan.targetWordCount} characters`} />
              ...
              {scenePlan.scenes.map((scene, index) => (
                <div key={scene.key} className="rounded-2xl border bg-background p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="secondary">{scene.targetWordCount} characters</Badge>
                  </div>
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <PanelHintCard title="starting state" content={scene.entryState} />
            ...
            <div className="rounded-2xl border bg-muted/20 p-4">
              <div className="text-xs text-muted-foreground">Scene teardown</div>
              <div className="mt-3 whitespace-pre-wrap text-sm leading-7">
                {selectedChapter.sceneCards?.trim()
                  ? "The current version is an old version of the scene disassembly text. It is recommended to regenerate the chapter execution contract."
                  : "There is no scene breakdown yet."}
              </div>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="quality" className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <MetricBadge label="style of writing" value={String(chapterQualityReport?.voice ?? "-")} />
        ...
        <TabsContent value="repair" className="space-y-3">
          {repairingOtherChapter ? (
            <ReferenceNotice
              ...
              description={`${repairStreamingChapterLabel ?? "Another chapter"} is still under repair. This chapter will not show that repair stream. Open the other chapter to keep watching.`}
            />
          ) : null}
          ...
          <div className="rounded-2xl border bg-muted/20 p-4">
            ...
            <div className="mt-3 max-h-[420px] overflow-y-auto whitespace-pre-wrap text-sm leading-7">
              {selectedChapter.repairHistory?.trim() || "There is no repair record yet."}
            </div>
          </div>
        </TabsContent>
