...
export default function KnowledgePage() {
  ...
  const [ragForm, setRagForm] = useState<KnowledgeEmbeddingSettingsFormState>({
    ...
    neo4jUser: "neo4j",
    neo4jPassword: "",
    neo4jPasswordConfigured: false,
    clearNeo4jPassword: false,
    neo4jTimeoutMs: 15000,
    vectorCandidates: 40,
  ...
  useEffect(() => {
    ...
    setRagForm({
      ...
      chunkWordSize: data.chunkWordSize,
      chunkOverlapWords: data.chunkOverlapWords,
      graphEnabled: data.graphEnabled,
      neo4jUri: data.neo4jUri,
      neo4jUser: data.neo4jUser,
      neo4jPassword: "",
      neo4jPasswordConfigured: data.neo4jPasswordConfigured,
      clearNeo4jPassword: false,
      neo4jTimeoutMs: data.neo4jTimeoutMs,
  ...
  const deleteRagJobMutation = useMutation({
    ...
    onError: (error) => {
      setRagJobsActionMessage(error instanceof Error ? error.message : "Delete task failed.");
    },
  });
  ...
  const ragHealthNotice = ragHealthQuery.isError
    ...
    : (ragHealthQuery.data?.message && ragHealthQuery.data.message !== "RAG health check passed."
      ? (ragHealthQuery.data.message === "RAG health check failed."
        ? "Data retrieval connection check failed."
  ...
  const documentListErrorMessage = documentsQuery.isError
    ? (documentsQuery.error instanceof Error ? documentsQuery.error.message : "Failed to load knowledge material.")
    : undefined;
  ...
  const handleSaveRagSettings = () => {
    saveRagMutation.mutate({
      ...
      chunkWordSize: ragForm.chunkWordSize,
      chunkOverlapWords: ragForm.chunkOverlapWords,
      graphEnabled: ragForm.graphEnabled,
      neo4jUri: ragForm.neo4jUri.trim(),
      neo4jUser: ragForm.neo4jUser.trim() || undefined,
      neo4jPassword: ragForm.neo4jPassword.trim() || undefined,
      clearNeo4jPassword: ragForm.clearNeo4jPassword,
      neo4jTimeoutMs: ragForm.neo4jTimeoutMs,
    ...
    <div className="space-y-5">
      ...
      >
        <TabsList className="h-11 w-full justify-start gap-1 overflow-x-auto rounded-full bg-muted/30 p-1">
          <TabsTrigger value="documents" className="rounded-full px-5">Creative materials</TabsTrigger>
          <TabsTrigger value="ops" className="rounded-full px-5">Indexes and tasks</TabsTrigger>
          <TabsTrigger value="settings" className="rounded-full px-5">Retrieve settings</TabsTrigger>
        </TabsList>
