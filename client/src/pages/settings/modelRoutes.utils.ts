...
export function formatStructuredStatus(status: ModelRouteConnectivityStatus["structured"]): string {
  if (!status) {
    return "Structured diagnostics: not executed";
  }
  if (status.ok) {
    return `Structured Normal · ${status.requestProtocol ?? "auto"} · ${status.strategy ?? "prompt_json"}${status.reasoningForcedOff ? " · Will turn off thinking" : ""}`;
  }
  return `Structured exceptions · ${status.errorCategory ?? "unknown"} · ${status.error ?? "unknown error"}`;
}

export function formatConnectivityStatus(status?: ModelRouteConnectivityStatus | null): string {
  if (!status) {
    return "No effective route has been detected yet.";
  }
  const parts: string[] = [];
  if (status.plain) {
    parts.push(
      status.plain.ok
        ? `Normal connection${status.plain.latency != null ? ` · ${status.plain.latency}ms` : ""}`
        : `Ordinary connection failure · ${status.plain.error ?? "unknown error"}`,
    );
  }
