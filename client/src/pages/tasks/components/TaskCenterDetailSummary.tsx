...
export default function TaskCenterDetailSummary({
  ...
  const technicalRows = [
    ["recent heartbeat", formatDate(task.heartbeatAt)],
    ["start time", formatDate(task.startedAt)],
    ["end time", formatDate(task.finishedAt)],
    ["Number of retries", task.retryCountLabel],
    ...((task.provider || task.model) ? [["call model", `${task.provider ?? "None"} / ${task.model ?? "None"}`]] : []),
    ...(isAutoDirectorTask ? [["interface model", currentModelLabel]] : []),
    ...((task.tokenUsage || task.provider || task.model) ? [
      ["Cumulative calls", formatTokenCount(task.tokenUsage?.llmCallCount ?? 0)],
      ["Enter Tokens", formatTokenCount(task.tokenUsage?.promptTokens ?? 0)],
      ["Output Tokens", formatTokenCount(task.tokenUsage?.completionTokens ?? 0)],
      ["Accumulated Tokens", formatTokenCount(task.tokenUsage?.totalTokens ?? 0)],
      ["recent records", formatDate(task.tokenUsage?.lastRecordedAt)],
    ] : []),
  ];
    ...
    <section className="space-y-4">
      ...
      <div className="grid gap-3 rounded-xl bg-muted/25 p-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
        ...
        {task.kind === "novel_workflow" ? (
          <>
            <div>
              <div className="text-[11px] text-muted-foreground">Suggest next steps</div>
              <div className="mt-1 leading-5 text-foreground/90">{task.resumeAction ?? task.nextActionLabel ?? "Continue the main flow of the novel"}</div>
            </div>
            <div>
              <div className="text-[11px] text-muted-foreground">latest checkpoint</div>
      ...
      <details className="group border-t border-border/35 pt-3">
        ...
        <dl className="mt-3 grid gap-x-4 gap-y-2 text-xs sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
          {task.kind === "novel_workflow" ? (
            <>
              <div><dt className="text-muted-foreground">Restore target page</dt><dd className="mt-0.5 text-foreground/85">{formatResumeTarget(task.resumeTarget)}</dd></div>
              <div><dt className="text-muted-foreground">Recent health stage</dt><dd className="mt-0.5 text-foreground/85">{task.lastHealthyStage ?? "None"}</dd></div>
            </>
          ) : null}
