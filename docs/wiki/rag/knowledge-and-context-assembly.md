...
- Knowledge-document chunking measures a **word budget**, not a character window. Default is 320 words with about 40 words of overlap from the last complete sentence. `Intl.Segmenter` splits sentences for English and Georgian; a word is never cut in the middle. The embedding token ceiling still shortens a chunk that would overflow the model.
- Runtime keys are `rag.chunkWordSize` and `rag.chunkOverlapWords`. Historical `rag.chunkSize` character values must not be reread as word counts.
- Georgian chapter queries stay Georgian for novel/world/character memory. Only the `knowledge_document` retrieval branch may rewrite a Georgian query into 1–3 English search phrases (`rag.knowledge_query.rewrite@v1`). Rewrite failure keeps the original query and must not stop chapter writing.
- English book excerpts stay English in `ragContext`. The chapter writer is told they may be English and must still write Georgian prose without copying another book's plot.
- Neo4j is a separate Docker service beside Qdrant. It stores people, places, themes, and techniques for English knowledge documents only, each linked to a chunk id. Graph sync is a `graph_sync` job after vector upsert. Neo4j or extraction failure must not mark the document index failed and must fail-open during retrieval.
- Search-index readiness and relationship readiness are separate marks on the knowledge document. `latestIndexStatus` and `lastIndexedAt` are written only by upsert, rebuild, and delete jobs. `latestGraphStatus` and `lastGraphSyncedAt` are written only by `graph_sync`. Clearing finished task records must not clear either mark. The knowledge task list and `GET /rag/jobs` return running jobs, then queued jobs, then recent finished jobs, so a long queue stays visible. A relationship job is titled with the book. While sections are read, progress reports the current section and the total. One failed section does not fail the book.
- Graph settings (`rag.graphEnabled`, URI, user, password, timeout) live on the knowledge-base RAG card. The password is stored like the Qdrant key and is never returned in full.
- The local Docker stack must start a persistent Qdrant service. The API container reaches it through the Compose service name `http://qdrant:6333`. `127.0.0.1` inside the container points only at the API container itself and cannot be used as a cross-container Qdrant address. Neo4j is started the same way: the API uses `bolt://neo4j:7687`, and Bolt is published to the host only as `127.0.0.1:17687`.
...
- RAG retrieval order is: vector recall and keyword recall in parallel, optional one-hop graph recall for English knowledge documents, RRF fusion, optional reranker, optional narrative-distance decay, then clip to finalTopK. Graph recall is an enhancement. Neo4j timeout or failure must fail-open and keep using vector and keyword hits.
- A knowledge question that contains two or more capitalized names keeps ordinary term ranking unless one name is at least three times rarer in the searched chunks than the next name. Keyword order then prefers passages that contain that rarer name, and hybrid fusion is otherwise unchanged. This is lexical evidence, not intent classification. Questions without those capitalized names, including Georgian questions, keep ordinary hybrid ranking.
...
- Recall quality is hard to reconstruct: check whether `RAG_RETRIEVAL_TRACE_SAMPLE_RATE` is 0, whether `RagRetrievalTrace` has recent records, whether `timingsJson` includes vector / keyword / graph / fusion / reranker / decay / total, and whether `fallbackTriggered` is true when facet hits are empty.
- A long knowledge question names a person, but the first passage does not contain that name: the names must be capitalized, and one name must be at least three times rarer in that book than the next name. Otherwise hybrid ranking is unchanged. A common title such as a rank or chapter word must not be treated as that rare name.
- Graph recall is always empty: check whether Neo4j is running, whether `rag.graphEnabled` is on, whether `latestGraphStatus` is `succeeded` for that English document, and whether the query rewrite produced English phrases. A graph outage must still leave vector hits. `latestIndexStatus` staying `succeeded` does not mean relationships were read.
- Relationship jobs disappear while older queued books remain: the task list must include running and queued knowledge jobs before finished ones. A relationship job title names the book. Progress that stays on loading source means the worker has not yet reported the current section.
