...
- A knowledge question that contains two or more capitalized names ranks keyword passages by how many of those names they contain, then by ordinary term frequency. After fusion, if the first passage is missing a name that the best keyword passage contains, and that keyword passage covers at least as many of the names, it is placed first. This is lexical evidence, not intent classification. Questions without those capitalized names, including Georgian questions, keep ordinary hybrid ranking.
...
- A long knowledge question names a person, but the first passage does not contain that name: the names must be capitalized in the query. Keyword order is name coverage, then term frequency. Fusion may still prefer a related passage; the name-bearing keyword passage is moved first only when it covers at least as many of those names and the leader is missing one. Uncapitalized questions stay on ordinary hybrid ranking.
- Graph recall is always empty: check whether Neo4j is running, whether `rag.graphEnabled` is on, whether `graph_sync` finished for that English document, and whether the query rewrite produced English phrases. A graph outage must still leave vector hits.
