...
test("normalizePlannerOutput coerces object-like planner fields into safe strings", () => {
  const normalized = normalizePlannerOutput({
    title: { main: "章节规划" },
    objective: ["推进主线", "强化冲突"],
    participants: ["Protagonist", { alias: "同伴" }],
    reveals: { first: "隐藏线索" },
    ...
    scenes: [
      {
        title: { text: "冲突爆发" },
        objective: { detail: "逼Protagonist做选择" },
        conflict: ["误解升级", "利益对撞"],
        reveal: { fact: "真相露出一角" },
        emotionBeat: 7,
      },
  ...
  assert.deepEqual(normalized.participants, ["Protagonist", "同伴"]);
  ...
  assert.equal(normalized.scenes[0].objective, "逼Protagonist做选择");
