...
const baseDisplayState = {
  ...
  stageLabel: "Beats / chapters",
  stepIndex: 4,
  totalSteps: 7,
  mode: "running",
  headline: "Auto-Directing",
  description: "AI 正在后台接管这本书的开书流程。",
  currentAction: "正在细化Chapter task sheet",
  checkpointLabel: "暂无",
  progressPercent: 45,
  nextActionLabel: null,
  currentFactStepId: "volume.chapter_detail_bundle.generate",
  currentFactStepLabel: "执行章节任务包细化",
  currentFactDescription: "正在细化Chapter task sheet",
  requiresUserAction: false,
  isLiveRunning: true,
  needsRecovery: false,
  steps: [
    { key: "project_setup", label: "Project setup", status: "completed", isCurrent: false },
    { key: "story_planning", label: "Story planning", status: "completed", isCurrent: false },
    { key: "character_setup", label: "Character setup", status: "completed", isCurrent: false },
    { key: "volume_strategy", label: "Volume strategy", status: "completed", isCurrent: false },
    { key: "structured_outline", label: "Beats / chapters", status: "running", isCurrent: true },
    { key: "chapter_execution", label: "Chapter execution", status: "pending", isCurrent: false },
    { key: "quality_repair", label: "Quality repair", status: "pending", isCurrent: false },
  ],
};
...
test("dashboard uses waiting mode only for explicit approval task checkpoints", () => {
  const view = buildView({
    task: {
      ...
      checkpointSummary: "前 10 章Auto-run is paused",
    },
    projection: {
      status: "waiting_approval",
      currentLabel: "Chapter execution等待确认",
      requiresUserAction: true,
...
test("completed dashboard prefers the task completion checkpoint over a trailing sync event", () => {
  const view = buildView({
    task: {
      ...
      checkpointSummary: "本轮Chapter execution、审核与Repairing已完成。",
    },
    projection: {
      status: "completed",
      currentLabel: "Sync character resource state完成。",
      requiresUserAction: false,
      policyMode: "auto_safe_scope",
      updatedAt: "2026-08-09T13:10:13.612Z",
      lastEventSummary: "Sync character resource state完成。",
      recentEvents: [],
      progressBreakdown: {
        totalPercent: 32,
        activeJobProgress: 1,
    ...
    displayState: {
      ...baseDisplayState,
      mode: "completed",
      currentAction: "Sync character resource state完成。",
      progressPercent: 32,
      isLiveRunning: false,
    },
    activeStep: null,
