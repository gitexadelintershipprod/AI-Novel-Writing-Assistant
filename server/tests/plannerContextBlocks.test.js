...
function createInput() {
  return {
    ...
    bible: "Protagonist必须靠主动布局而不Yes外挂碾压。",
    ...
    mappedVolumes: [
      {
        ...
        mainPromise: "Protagonist在高压环境下抢回一点主动权",
        climax: "卷末反压成立",
    ...
    characters: "Protagonist|核心角色|goal=抢回主动权|state=被压制",
    recentSummaries: "上一章：Protagonist踩中陷阱但看见反扑机会。",
    plotBeats: "5 第一次反压 卷内兑现",
    stateSnapshot: "Protagonist与敌人的资源差距仍然明显。",
    openAuditIssues: "plot/high: 上一轮缺少明确兑现",
    recentDecisions: "plot/high: 先兑现反压再拉大盘面",
    characterDynamicsSummary: "当前卷：第一卷\n当前卷核心角色：Protagonist、女二\n缺席高风险角色：女二(high, 缺席跨度=3)\n待确认候选：1 个",
    characterVolumeAssignments: "Protagonist | 卷级身份=破局者 | 卷级职责=完成第一次反压 | 计划章次=5\n女二 | 卷级身份=暗线持钥者 | 卷级职责=补足情报链 | 缺席风险=high(跨度=3)",
    characterRelationStages: "Protagonist -> 女二: 互试探合作 | 双方仍在试探底线 | 下一步=交换关键情报",
    ...
    stateDrivenGoal: [
      "章节状态目标：把反压推进到可见收益",
      "应推进冲突：第一次反压必须落地",
      "应推进关系：Protagonist: 互试探合作",
      "应触碰 payoff：女二情报钥匙",
      "禁止提前泄露：幕后黑手身份",
      "最近关键Event: Protagonist确认账本异常",
    ].join("\n"),
    defaultMetadata: "planRole=pressure | phase=反压前夜\nmustAdvance=第一次反压\nmustPreserve=压迫感",
    replanContext: "无",
    payoffLedgerSummary: [
      "账本Summary: 待兑现=2，紧急=1，逾期=1，已兑现=1",
      "当前未兑现项：女二情报钥匙 | 需要先铺垫她带来的反压价值；黑市账户异常 | Protagonist要先确认账本有异动",
      "当前逾期项：黑市账户异常 | 目标窗口已过，不能继续拖延",
      "本章应触碰项：女二情报钥匙 | 窗口=5-6；黑市账户异常 | 窗口=4-5",
      "最近一次已兑现项：第一次反压试探 | 已在第4章附近兑现",
    ].join("\n"),
    storyMacroSummary: "核心冲突：Protagonist在压迫中夺回主动权\n推进回路：每次反压都会引来更强反扑",
...
test("chapter planner context prioritizes framing, story macro and current volume window over legacy outline", () => {
  ...
  assert.match(byId.get("book_framing").content, /Theme base: Urban superpower/);
  ...
  assert.match(byId.get("story_macro").content, /核心冲突：Protagonist在压迫中夺回主动权/);
  ...
  assert.match(byId.get("character_dynamics_summary").content, /当前卷核心角色：Protagonist、女二/);
  ...
  assert.match(byId.get("payoff_ledger").content, /账本Summary: 待兑现=2，紧急=1，逾期=1，已兑现=1/);
