...
function createWorkspaceContext() {
  return {
    novel: {
      ...
      world: null,
      bookContract: {
        absoluteRedLines: ["不要改Protagonist核心动机"],
      },
      chapters: [{
        id: "chapter-1",
        title: "Test Chapter",
    ...
    chapterPlan: {
      ...
      mustPreserveJson: JSON.stringify(["保留Protagonist隐忍感"]),
    ...
    macroContext: {
      ...
      previousChapterBridge: "There is no previous-chapter summary to pick up before this chapter.",
      nextChapterBridge: "There is no next-chapter summary to reference after this chapter.",
      activePlotThreads: ["把冲突往前推", "身份逆转伏笔"],
      characterStateSummary: "- Lin Zhou is trying to stay composed. / stabilize the scene / tense",
      worldConstraintSummary: "No extra world constraints.",
      mustKeepConstraints: ["保留Protagonist隐忍感", "不要改Protagonist核心动机"],
    },
  };
}
...
test("GET workspace and POST ai-revision-preview routes return the new editor payloads", async () => {
  ...
  DefaultNovelApplicationServices.prototype.getChapterEditorWorkspace = async () => ({
    ...
    macroContext: {
      ...
      characterStateSummary: "Protagonist在忍",
