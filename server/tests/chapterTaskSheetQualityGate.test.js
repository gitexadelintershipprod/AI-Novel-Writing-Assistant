    ...
    scenes: [
      {
        key: "scene-1",
        title: "入口压力",
        purpose: "让Protagonist被迫正面处理新的资源危机。",
        mustAdvance: ["暴露危机来源"],
        mustPreserve: ["不提前解决最终对手"],
        entryState: "Protagonist刚拿到异常线索。",
        exitState: "Protagonist确认危机来自内部。",
      ...
      },
      {
        key: "scene-2",
        title: "主动试探",
        purpose: "让Protagonist用低成本方案试探对方底线。",
        mustAdvance: ["获得一个可验证证据"],
        mustPreserve: ["保留关系张力"],
        entryState: "Protagonist掌握第一条线索。",
        exitState: "对手被迫露出反常反应。",
        forbiddenExpansion: ["不要让冲突直接Resolution"],
        targetWordCount: 1000,
      },
      {
        key: "scene-3",
...
function buildCandidate(overrides = {}) {
  return {
    ...
    chapterOrder: 1,
    title: "第一章 危机入局",
    summary: "Protagonist发现资源危机并开始试探。",
    purpose: "推进Protagonist从被动承压转为主动试探。",
    exclusiveEvent: "Protagonist第一次确认资源危机来自内部。",
    endingState: "Protagonist拿到第一份证据。",
    nextChapterEntryState: "Protagonist带着证据进入下一轮试探。",
    conflictLevel: 45,
    revealLevel: 35,
    targetWordCount: 3000,
    mustAvoid: "不要提前揭示幕后主使，不要复写下一章核心事件。",
    payoffRefs: ["资源危机"],
    taskSheet: "本章以资源危机开场，Protagonist从被动承压转为主动试探，结尾留下更危险的证据入口。",
...
test("chapter task sheet quality service lets full book mode auto-repair semantic failures", async () => {
  const service = new ChapterTaskSheetQualityGateService(async () => ({
    verdict: "repairable",
    safeToSync: false,
    summary: "任务单可Repairing，但当前版本仍会越界。",
    issues: [
      {
...
test("chapter task sheet quality prompt declares strict JSON contract", () => {
  ...
  assert.match(systemText, /Only usable, repairable, and unusable can be used for verdict/);
  assert.match(systemText, /issues\.target can only use purpose, boundary, task_sheet, scene_cards, and semantic/);
  assert.match(systemText, /readerExperience\.rewardLevel represents the visible reward intensity/);
  assert.match(systemText, /can only be used with setup, partial, and major/);
  assert.match(systemText, /not recommended to change rewardLevel to full/);
  assert.match(systemText, /confidence must be a decimal between 0 and 1/);
...
test("chapter task sheet quality service marks overloaded contracts for window replan", async () => {
  const service = new ChapterTaskSheetQualityGateService(async () => ({
    ...
    summary: "本章同时承担多条 payoff 和多名角色Turn，职责过载。",
    issues: [],
    repairGuidance: ["把其中一条 payoff 和一个角色Turn拆到下一章。"],
    confidence: 0.86,
  }));
