...
function buildDb(overrides = {}) {
  ...
  const chapter = {
    ...
    title: "旧仓库里的信号",
    order: 3,
    content: "Protagonist走进旧仓库，发现信号记录。",
    expectation: "让Protagonist发现关键线索。",
    taskSheet: "必须推进：发现档案站信号。必须保留：队友仍然怀疑Protagonist。",
    ...
    updatedAt: date(),
    chapterSummary: {
      summary: "Protagonist发现旧信号。",
      keyEvents: "进入旧仓库；发现信号。",
      characterStates: "Protagonist开始怀疑公司。",
    },
  };
  ...
  return {
    ...
    novelWorkflowTask: {
      ...
      findFirst: async () => ({
        id: "task-1",
        title: "Auto-Director推进",
        status: "running",
        progress: 0.4,
        currentStage: "chapter_pipeline",
        currentItemLabel: "第 3 章",
        checkpointSummary: "Chapter execution中。",
        lastError: null,
        updatedAt: date(),
      }),
    },
