...
test("chapter execution contract requires reader experience and scene experience fields", () => {
  ...
  const scene = (index) => ({
    ...
    resistance: "敌方施加具体阻力。",
    turn: "Protagonist改变策略并夺回一步主动。",
    emotionalShift: "压迫转为反击期待。",
    readerValue: "读者获得推进和局部回报。",
  });
  const base = {
    purpose: "完成第一次反压。",
    exclusiveEvent: "Protagonist第一次迫使敌方后退。",
    endingState: "Protagonist获得局部主动。",
    ...
    payoffRefs: [],
    taskSheet: "让Protagonist主动试探并拿到可见收益。",
  ...
  assert.equal(schema.safeParse({
    ...base,
    readerExperience: {
      readerQuestion: "Protagonist能No完成第一次反压？",
      promisedReward: "Protagonist拿到第一次可见主动权。",
      rewardLevel: "partial",
      protagonistWant: "迫使敌方后退。",
      primaryResistance: "敌方掌握资源优势。",
      keyTurn: "Protagonist识破并利用敌方漏洞。",
      emotionalShift: "受压转为反击快感。",
      informationReveal: "敌方封锁存在漏洞。",
      netChange: "Protagonist从被动转为局部主动。",
...
function makeResourceDelta(index = 1) {
  return {
    ...
    readerKnows: true,
    holderKnows: true,
    knownByCharacterNames: ["Protagonist"],
    narrativeFunction: "key",
    summary: `Protagonist获得关键资源${index}。`,
    narrativeImpact: "影响后续行动边界。",
    evidence: [`Protagonist收起关键资源${index}。`],
    confidence: 0.9,
    riskLevel: "low",
  };
}
...
test("chapter acceptance schema accepts obligation diagnostics", () => {
  const parsed = chapterAcceptanceAssessmentSchema.parse({
    ...
    decisionReason: "局部补写即可Repairing，不需要重排章节。",
    riskTags: ["missing_character_appearance"],
    assetSyncRecommendation: {
      priority: "normal",
      reason: "Repairing后可继续普通同步。",
      requiresFullPayoffReconcile: false,
    },
    continuePolicy: "repair_once",
  });
...
test("chapter acceptance schema normalizes obligation and policy aliases", () => {
  const parsed = chapterAcceptanceAssessmentSchema.parse({
    ...
    repairDirectives: [{
      mode: "fix",
      target: "plot",
      instruction: "补出敌方试探和Protagonist识破。",
    }],
    missingObligations: [{
...
test("chapter artifact delta schema normalizes common LLM aliases from extraction output", () => {
  const parsed = chapterArtifactDeltaOutputSchema.parse({
    summary: "本章推进多条伏笔。",
    stateDeltas: {
      summary: "Protagonist完成表白并发现陷害计划。",
      characterStates: [],
      relationStates: [],
      informationStates: [],
      foreshadowStates: [],
