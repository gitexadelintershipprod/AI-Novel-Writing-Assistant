...
test("normalizeNovelBiblePayload flattens object bible fields into strings", () => {
  const payload = normalizeNovelBiblePayload({
    coreSetting: {
      premise: "抗日Background下的奇侠传奇",
      hook: "民间秘术对抗侵略势力",
    },
    forbiddenRules: ["不能牺牲主线成长", "不能破坏家国主题"],
    mainPromise: {
      summary: "Protagonist在乱世中守护家园并成长为领袖",
    },
    characterArcs: {
  ...
  assert.match(payload.coreSetting ?? "", /premise: 抗日Background下的奇侠传奇/);
  assert.equal(payload.forbiddenRules, "不能牺牲主线成长\n不能破坏家国主题");
  assert.equal(payload.mainPromise, "Protagonist在乱世中守护家园并成长为领袖");
