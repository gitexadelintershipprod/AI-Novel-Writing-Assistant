...
function installPipelineStubs() {
  const state = {
    episode: {
      ...
      emotionNet: 4,
      isPaywall: false,
      beatSheet: JSON.stringify({ conflict: "Protagonist隐藏身份被羞辱" }),
  ...
  function buildProject() {
    return {
      ...
      source: "text_import",
      sourceRef: null,
      sourceInput: "Protagonist隐藏身份进入公司。",
      ...
      sourceBundle: {
        ...
        beats: JSON.stringify([{ order: 1, summary: "Protagonist隐藏身份入职，被同事羞辱。" }]),
        worldNotes: "现代都市公司。",
        hardFacts: JSON.stringify([{ text: "Protagonist真实身份Yes集团继承人", category: "revealed" }]),
        rawText: null,
      },
      characters: [{
        id: "character_1",
        ...
        if (asset.id === "drama.episode.script") {
          return {
            output: {
              ...
              durationSec: 72,
              sceneCount: 2,
              opening3s: "Protagonist被保安拦下并被嘲讽",
