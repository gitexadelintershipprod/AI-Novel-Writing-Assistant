...
const ragSettingsSchema = z.object({
  ...
  chunkWordSize: z.coerce.number().int().min(80).max(800),
  chunkOverlapWords: z.coerce.number().int().min(0).max(200),
  graphEnabled: z.boolean(),
  neo4jUri: z.string().trim().min(1),
  neo4jUser: z.string().trim().min(1).optional(),
  neo4jPassword: z.string().optional(),
  clearNeo4jPassword: z.boolean().optional(),
  neo4jTimeoutMs: z.coerce.number().int().min(1000).max(120000),
    ...
    try {
      ...
      const [embeddingResult, runtimeResult] = await Promise.all([
        ...
        saveRagRuntimeSettings({
          ...
          chunkWordSize: body.chunkWordSize,
          chunkOverlapWords: body.chunkOverlapWords,
          graphEnabled: body.graphEnabled,
          neo4jUri: body.neo4jUri,
          neo4jUser: body.neo4jUser,
          neo4jPassword: body.neo4jPassword,
          clearNeo4jPassword: body.clearNeo4jPassword,
          neo4jTimeoutMs: body.neo4jTimeoutMs,
