import type { ChapterExecutionObligationContract, ChapterWriteContext, } from "@ai-novel/shared/types/chapterRuntime";
...
export function normalizeChapterWriteContext(writeContext: ChapterWriteContext): ChapterWriteContext {
    ...
    const compatibleReaderExperience: ReaderExperienceContract = hasReaderExperienceContractValue(storedReaderExperience)
        ...
        : {
            ...
            protagonistWant: writeContext.participants.find((item) => item.role === "Protagonist")?.currentGoal
            ...
            informationReveal: "This chapter delivers only the necessary information that the task allows.",
...
export function selectCharacterHardFactsForWriter(input: {
    ...
    for (const guide of input.characterBehaviorGuides) {
        if (guide.shouldPreferAppearance
