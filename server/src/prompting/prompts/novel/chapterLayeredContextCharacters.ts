...
export function buildDynamicCharacterGuidance(contextPackage: GenerationContextPackage): Pick<ChapterWriteContext, "characterBehaviorGuides" | "activeRelationStages" | "pendingCandidateGuards"> {
    ...
    const dialogueGuidanceByCharacterId = new Map((contextPackage.characterDialogueGuidances ?? []).map((guidance) => [guidance.characterId, guidance]));
    const planParticipantNames = new Set((contextPackage.plan?.participants ?? []).map((item) => compactText(item)));
    const conflictCharacterIds = new Set(contextPackage.openConflicts.flatMap((conflict) => conflict.affectedCharacterIds ?? []));
    ...
    const characterBehaviorGuides = overview.characters
        ...
        const shouldPreferAppearance = item.isCoreInVolume && (item.plannedChapterOrders.includes(currentChapterOrder)
        ...
        return {
            ...
            guide: {
                ...
                relationStageLabels: takeUnique(relationStages.map((relation) => (relation.nextTurnPoint
                    ? `${relation.stageLabel} -> ${relation.nextTurnPoint}`
                    : relation.stageLabel)), 3),
                relationRiskNotes: takeUnique(relationStages.map((relation) => (`${relation.sourceCharacterName} / ${relation.targetCharacterName}: ${relation.stageSummary}${relation.nextTurnPoint ? ` | next=${relation.nextTurnPoint}` : ""}`)), 3),
...
function buildMindGuidance(mind: GenerationContextPackage["characterMindStates"][number] | undefined): string | null {
    ...
    const parts = takeUnique([
        mind.currentInterpretation ? `Role understanding:${compactText(mind.currentInterpretation)}` : "",
        mind.activePlan ? `Tendency to act:${compactText(mind.activePlan)}` : "",
        mind.actionTendency ? `Stress response:${compactText(mind.actionTendency)}` : "",
        mind.misbeliefs[0] ? `Possible misjudgment:${compactText(mind.misbeliefs[0])}` : "",
    ], 3);
    return parts.length > 0 ? parts.join(" | ") : null;
}
function buildDialogueInfluenceGuidance(influence: NonNullable<GenerationContextPackage["characterDialogueGuidances"]>[number] | undefined): string | null {
    if (!influence) {
        return null;
    }
    const parts = takeUnique([
        influence.behaviorGuidance ? `Action tendencies:${compactText(influence.behaviorGuidance)}` : "",
        influence.emotionalGuidance ? `Emotional tendencies:${compactText(influence.emotionalGuidance)}` : "",
        influence.relationTension ? `Relationship tension:${compactText(influence.relationTension)}` : "",
        influence.summary ? `Conversation precipitation:${compactText(influence.summary)}` : "",
    ], 3);
    return parts.length > 0 ? parts.join(" | ") : null;
}
export function buildParticipants(contextPackage: GenerationContextPackage, characterBehaviorGuides: ChapterWriteContext["characterBehaviorGuides"] = []): GenerationContextPackage["characterRoster"] {
    const rosterById = new Map(contextPackage.characterRoster.map((character) => [character.id, character]));
    ...
    const selected = contextPackage.characterRoster.filter((character) => (participantNames.has(character.name) || conflictCharacterIds.has(character.id)));
    if (selected.length > 0) {
        return selected.slice(0, 6);
    }
