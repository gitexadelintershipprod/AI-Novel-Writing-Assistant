import type { VolumeBeat, VolumeBeatSheet, VolumeCountGuidance, VolumePlan, VolumeStrategyPlan, } from "@ai-novel/shared/types/novel";
import { parseChapterScenePlan } from "@ai-novel/shared/types/chapterLengthControl";
import type { StoryMacroPlan } from "@ai-novel/shared/types/storyMacro";
import type { ChapterDetailMode, VolumeGenerationNovel, VolumeWorkspace, } from "../../../../services/novel/volume/volumeModels";
...
function summarizeCharacters(novel: VolumeGenerationNovel): string {
    ...
    return novel.characters
        .slice(0, 6)
        .map((item) => (`${item.name} | ${item.role} | goal=${item.currentGoal ?? "none"} | state=${item.currentState ?? "none"}`))
        .join("\n");
}
...
export function buildWindowedVolumeContext(volumes: VolumePlan[], targetVolumeId?: string, windowSize = 1): string {
    if (volumes.length === 0) {
        return "none";
    }
...
export function buildSoftFutureVolumeSummary(volumes: VolumePlan[], targetVolumeId?: string): string {
    if (!targetVolumeId) {
        return "none";
    }
    const sorted = volumes
...
function describeConflictStep(previous: number | null | undefined, current: number | null | undefined): string {
...
export function buildConflictLevelCurveContext(volume: VolumePlan, targetChapterId?: string, options: {
    ...
    return sortedChapters.map((chapter, index) => {
        ...
        return [
            ...
            isUserAnchored ? "constraint=user-anchored, cannot be changed" : "",
        ].filter(Boolean).join(" | ");
    }).join("\n");
}
...
function buildSceneTrajectoryLine(chapter: VolumePlan["chapters"][number], sceneIndex: number, label: string): string | null {
...
export function buildRecentChapterExecutionContext(volume: VolumePlan, chapterId: string, lookback = 2): string {
    const index = volume.chapters.findIndex((chapter) => chapter.id === chapterId);
    if (index <= 0) {
        return "none";
    }
    ...
    return recentChapters.map((chapter) => {
        ...
        const middleSceneLines = scenePlan && scenePlan.scenes.length > 2
            ? scenePlan.scenes
                .slice(1, -1)
                .map((scene, middleIndex) => (`middle scene ${middleIndex + 1}: ${scene.title} | purpose=${scene.purpose} | mustAdvance=${scene.mustAdvance.join(" | ") || "none"}`))
            : [];
        return [
...
export function buildChapterDetailDraft(chapter: VolumePlan["chapters"][number], detailMode: ChapterDetailMode): string {
