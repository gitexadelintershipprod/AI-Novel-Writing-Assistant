...
export const directorIdeaConstellationOptionsSchema = z.object({
    ...
    if (ids.size !== output.options.length) {
        context.addIssue({ code: "custom", message: "Story star chart option id cannot be repeated." });
    }
    for (const category of DIRECTOR_IDEA_CONSTELLATION_CATEGORIES) {
        ...
        if (count !== 5) {
            context.addIssue({ code: "custom", message: `Story Star Chart Category ${category} Must contain exactly 5 items.` });
        }
    }
});
