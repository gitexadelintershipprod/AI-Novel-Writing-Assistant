import type { BookContractContext, ChapterMissionContext, ChapterRepairContext, ChapterReviewContext, ChapterWriteContext, GenerationContextPackage, MacroConstraintContext, PromptBudgetProfile, VolumeWindowContext, } from "@ai-novel/shared/types/chapterRuntime";
import { parseChapterScenePlan, resolveLengthBudgetContract, } from "@ai-novel/shared/types/chapterLengthControl";
import { sanitizeCreativeMustAdvanceItems } from "@ai-novel/shared/types/chapterCreativeContract";
import type { ReviewIssue } from "@ai-novel/shared/types/novel";
import type { StoryMacroPlan } from "@ai-novel/shared/types/storyMacro";
import { hasReaderExperienceContractValue, normalizeReaderExperienceContract, type ReaderExperienceContract, } from "@ai-novel/shared/types/novel/readerExperience";
import { createContextBlock } from "../../core/contextBudget";
import type { PromptContextBlock } from "../../core/promptTypes";
import { buildDynamicCharacterGuidance, buildParticipants } from "./chapterLayeredContextCharacters";
import { buildCharacterGuidanceText, buildLedgerItemLine, buildParticipantText, buildPendingCandidateGuardText, buildRelationStageText, compactText, renderBookContractText, renderStoryMacroText, resolveTargetWordRange, splitLines, summarizeContinuationConstraints, summarizeHistoricalIssues, summarizeOpenConflicts, summarizeStateSnapshot, summarizeStyleConstraints, summarizeWorldRules, takeUnique, toListBlock, } from "./chapterLayeredContextShared";
...
import { normalizeChapterWriteContext, selectCharacterHardFactsForWriter, } from "./context/chapterContextPolicies";
export { WRITER_FORBIDDEN_GROUPS, buildChapterRepairContextBlocks, buildChapterReviewContextBlocks, buildChapterWriterContextBlocks, sanitizeWriterContextBlocks, } from "./context/chapterContextBlocks";
...
function buildCompatibleReaderExperienceContract(input: {
    ...
    const protagonist = input.participants.find((item) => item.role === "Protagonist") ?? input.participants[0] ?? null;
    ...
    return {
        ...
        primaryResistance: input.openConflictSummaries[0]
            ...
            || "There are specific obstacles and costs that must be faced in completing the tasks in this chapter.",
        ...
        informationReveal: input.contextPackage.plan?.reveals[0] || "This chapter delivers only the necessary information that the task allows.",
...
export function buildChapterMissionContext(contextPackage: GenerationContextPackage): ChapterMissionContext {
    ...
    return {
        ...
        objective: compactText(stateGoal?.summary)
        ...
        expectation: compactText(contextPackage.chapter.expectation)
...
export function buildNarrativeProgressHint(currentOrder: number, estimatedTotal: number | null | undefined): string | null {
    if (!estimatedTotal || estimatedTotal <= 0)
        return null;
    const progress = currentOrder / estimatedTotal;
    const remaining = estimatedTotal - currentOrder;
    if (progress < 0.25) {
        return `[Narrative Progress] Chapter ${currentOrder} Chapters / Estimated total ${estimatedTotal} Chapter (${Math.round(progress * 100)}%)
The opening stage: the world and characters can be freely developed to build readers\u2019 expectations.`;
    }
    if (progress < 0.75) {
        return `[Narrative Progress] Chapter ${currentOrder} Chapters / Estimated total ${estimatedTotal} Chapter (${Math.round(progress * 100)}%)
Development stage: Advance existing clues, cautiously open new branches, and maintain the density of foreshadowing.`;
    }
    if (progress < 0.90) {
        return `[Narrative Progress] Chapter ${currentOrder} Chapters / Estimated total ${estimatedTotal} Chapter (${Math.round(progress * 100)}%)
Convergence stage: Prioritize cashing out the foreshadowing and avoid opening new main lines. There is still some time before the end. ${remaining} Chapter.`;
    }
    return `[Narrative Progress] Chapter ${currentOrder} Chapters / Estimated total ${estimatedTotal} Chapter (${Math.round(progress * 100)}%)
Ending stage: wrap up all the main lines and conclude the book. It is forbidden to open new branch lines.`;
}
function buildChapterBoundaryContract(contextPackage: GenerationContextPackage, scenePlan: ReturnType<typeof parseChapterScenePlan>): ChapterWriteContext["chapterBoundary"] {
    ...
    const doNotCross = takeUnique([
        ...
        ...protectedReveals.map((item) => `Not disclosed in advance:${item}`),
        ...scenes.flatMap((scene) => scene.forbiddenExpansion ?? []),
        lastScene?.exitState ? `Do not go beyond the end of this chapter:${lastScene.exitState}` : "",
        contextPackage.chapter.hook ? `Subsequent events after the hook must not be directly expanded:${contextPackage.chapter.hook}` : "",
    ], 12).filter(Boolean);
    return {
...
export function buildChapterWriteContext(input: {
    ...
    const characterBehaviorGuides = dynamicCharacterGuidance.characterBehaviorGuides.map((guide) => (participantIds.has(guide.characterId)
        ? guide
...
function buildChapterExecutionObligationContract(input: {
    ...
    return {
        ...
        requiredPayoffTouches: uniqueStrings(input.payoffDirectives.map((item) => (`${item.operation}: ${item.title}`))),
        requiredCharacterAppearances: uniqueStrings(input.characterBehaviorGuides
            .filter((guide) => (guide.shouldPreferAppearance
            || guide.plannedChapterOrders.includes(input.chapterOrder)))
            .map((guide) => {
            if (guide.absenceRisk === "high" && guide.absenceSpan > 0) {
                return `${guide.name}(Absent ${guide.absenceSpan} Chapter, should be brought out naturally)`;
            }
            return guide.name;
        })),
...
export function buildChapterReviewContext(writeContext: ChapterWriteContext, contextPackage: GenerationContextPackage): ChapterReviewContext {
...
export function buildChapterRepairContext(input: {
    ...
    return {
        ...
        allowedEditBoundaries: takeUnique([
            ...
            ...writeContext.ledgerOverdueItems.map((item) => `You must either cash out or explicitly explain the overdue payoff pressure: ${item.title}`),
...
export function buildChapterRepairContextFromPackage(contextPackage: GenerationContextPackage, issues: ReviewIssue[]): ChapterRepairContext | null {
...
export function withChapterRepairContext(contextPackage: GenerationContextPackage, issues: ReviewIssue[]): GenerationContextPackage {
