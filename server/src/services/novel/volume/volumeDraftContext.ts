...
function normalizeChapterInput(raw: unknown, fallbackOrder: number): Omit<VolumeChapterPlan, "chapterOrder" | "volumeId"> & { inputOrder: number } {
  ...
  return {
    ...
    title: normalizeText(record.title) ?? `Chapter ${inputOrder}`,
    summary: normalizeText(record.summary) ?? normalizeText(record.purpose) ?? "Chapter summaries waiting to generate",
  ...
  return rawVolumes
    .map((rawVolume, volumeIndex) => {
      ...
      return {
        ...
        novelId,
        sortOrder,
        title: normalizeText(record.title) ?? `Volume ${sortOrder}`,
