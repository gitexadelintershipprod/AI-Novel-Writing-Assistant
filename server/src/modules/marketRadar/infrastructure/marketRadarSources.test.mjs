...
test("parses public Fanqie ranking metadata without chapter content", () => {
  ...
  const items = parseFanqieRanking(html, source("fanqie", "reading", "Most Read", "https://fanqienovel.com/rank"));
...
test("parses Qidian list rank, author and category", () => {
  ...
  <p class="subTitle_x">某作者 <em>·</em> Suspense <em>·</em> 80万字</p></div></a>`;
  const items = parseQidianRanking(html, source("qidian", "monthly_ticket", "Monthly Votes", "https://m.qidian.com/rank/yuepiao/"));
  assert.equal(items.length, 1);
  assert.equal(items[0].author, "某作者");
  assert.equal(items[0].category, "Suspense");
});

test("parses Jinjiang public monthly titles", () => {
  const html = `<ul><li><a href="/book2/6836927" class="track">在古代开医馆</a></li></ul>`;
  const items = parseJinjiangRanking(html, source("jinjiang", "monthly", "Monthly Ranking", "https://m.jjwxc.net/rank/naturalmore/5"));
  assert.equal(items.length, 1);
